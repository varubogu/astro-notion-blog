...
export async function getAllPostComments(): Promise<PostComment[]> {
  ...
  if (!dbResponse || dbResponse.in_trash) {
    console.error(
      'The database either does not exist or is in trash. Please restore it to fetch postComments.'
    )
    return []
  }
  ...
  if (!dataSouceId) {
    console.error(
      'No data source found for the database. Please add a data source to fetch postComments.'
    )
    return []
  }
